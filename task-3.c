/*	
	...
	and selects the smaller one (which is more urgent) to proceed first. 
	If current one is more urgent, it still runs. But if new one is more urgent, current one will be stopped and new one will run prefentially.
    ...
    while (1)
    {
        ...
        while (pcb_index < count)
        {
            ...
            if (pcb[pcb_index].state == READY)
            {
                ...
                //current process keeps going until it is stopped by another process which has less time till deadline
                int remain_time = pcb[pcb_index].remaining_time;
                for (int i = 0; i < remain_time; i++)
                {
                    ...
                    //check whether the current process needs to keep going
                    ...
                    for (int i = pcb_index + 1; i < count; i++)
                    {
                        ...
                        if (pcb[i].entry_time == current_time)
                        {
                            ...
                            //compare the results of service_time - ddl_time of the current process and the waiting process, proceed the smaller one
                            if (pcb[i].service_time < pcb[pcb_index].service_time && (pcb[i].deadline_time - pcb[i].service_time) <= (pcb[pcb_index].deadline_time - pcb[pcb_index].service_time))
                            {
                                mark = 1;
                                //reset pcb
                                pcb[pcb_index].state = READY;
...
//delete a certain character, return a new pointer and points to a new string串
